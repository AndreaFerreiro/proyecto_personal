...
const Views = require('../views/notesViews');
const List =  async (req,res) => {
    ...
    try{
        const result = await Notes.find(conditions);
        console.log(result);
        res.json(result);
    }
    catch(error){
...
const Create = async (req,res) => {
    try{
        const doc = await Notes.create(req.body);
        console.log(doc);
...
const Update = async (req,res)=> {
    try{
        const newProduct = await Notes.findOneAndUpdate({_id:req.params.productId},{$set:req.body},{new:true});
        if (newProduct ===null){
...
const Delete = async (req, res)=> {
    try{
        const deleteProduct = await Notes.deleteOne({_id:req.params.productId});
